 // ---------------------------
    ...
    function renderPost(post) {
      ...
      const meta = document.createElement('div');
      meta.className = 'meta';
      const [datePart, timePart, ampm] = post.time.split(/[\s]/);
      meta.innerHTML = iconCalendar() + ` <span>${datePart}</span> · ` + iconClock() + ` <span>${timePart} ${ampm}</span>`;
      ...
      (post.comments || []).forEach((c, cIdx) => {
        ...
        cm.innerHTML = `
      <div class="avatar">${initials(c.name || 'A')}</div>
      <div style="flex:1">
        <div class="body">
          <div class="meta">${escapeHtml(c.name)} · ${c.time}</div>
          <div class="text">${escapeHtml(c.text)}</div>
        </div>
      </div>
    `;

        // Fan or Admin options
        if (isAdmin()) {
          // Admin: Edit/Delete any comment
          const editAdminBtn = document.createElement('button');
          editAdminBtn.className = 'icon-btn';
          editAdminBtn.textContent = 'Edit';
          editAdminBtn.addEventListener('click', () => {
            ...
            if (pIdx >= 0) {
              posts[pIdx].comments[cIdx].text = newText;
              savePosts(posts); render();
            }
          });

          const deleteAdminBtn = document.createElement('button');
          deleteAdminBtn.className = 'icon-btn';
          ...
          cm.appendChild(editAdminBtn);
          cm.appendChild(deleteAdminBtn);

        } else if (c.isOwn) {
          // Fan: Edit/Delete their own comment
          const editFanBtn = document.createElement('button');
          editFanBtn.className = 'icon-btn';
          editFanBtn.textContent = 'Edit';
          editFanBtn.addEventListener('click', () => {
          ...
          cm.appendChild(editFanBtn);

          const deleteFanBtn = document.createElement('button');
          deleteFanBtn.className = 'icon-btn';
          deleteFanBtn.textContent = 'Delete';
          deleteFanBtn.addEventListener('click', () => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (idx >= 0) {
          posts[idx].comments = posts[idx].comments || [];
          posts[idx].comments.push({ name, text, time: nowText(), isOwn: true, replies: [] });
          savePosts(posts); render();
        }
      });
      commentForm.appendChild(inName); commentForm.appendChild(inText); commentForm.appendChild(btn);
